...
export class AddModalComponent {
    ...
    public validationForm = new FormGroup({
        remplacant: new FormControl('', Validators.required),
        remplace: new FormControl('', Validators.required),
        motif: new FormControl('', Validators.required),
        date_debut: new FormControl('', Validators.required),
        date_fin: new FormControl('', Validators.required),
        idinterim: new FormControl('',),
    ...
    submitForm() {
        ...
        let payload = {
            ...
            "action": this.validationForm.value.idinterim ? 2 : 1,
            "idinterim": this.validationForm.value.idinterim || '',
            "idsociete": this.users?.datasociete?.uid,
        }

        console.log("payload ===", payload)
        this.httService.postData(`${environment.api_url}auth/:saveinterim`, payload, this.users?.access_token || '')
            .toPromise()
            .then((res: any) => {
        ...
        this.httService.getData(`${environment.api_url}auth/:liste-des-comptes?idsociete=${idsociete}&idpersonnel=${idpersonnel}`, false, this.users?.access_token || '')
            ...
            .then((res: any) => {
                ...
                if (res.body.status) {
                    ...
                    this.dataRemplacant = res.body.data.map((e: any) => {
                        return {
                            ...e,
                    ...
                    console.log("this.filteredData ===", this.dataRemplacant)
    ...
    selectInterim(event: any) {
        this.validationForm.get('remplace')?.setValue('');
        this.validationForm.get('remplace')?.markAsUntouched();
        this.dataRemplace = this.dataRemplacant.filter((d: any) => d.value != event.value);
    }
}
