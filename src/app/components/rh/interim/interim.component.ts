...
export class InterimComponent implements OnInit {
    ...
    filters: {
        ...
        motif: { text: string; value: string }[];
    ...
    filterFns = {
        ...
        motif: (list: string[], item: RowData) =>
            list.some(val =>
                (item.motif ?? '').toLowerCase().includes(val.toLowerCase())
            ),
    ...
    ngOnInit(): void {
        ...
        this.showInterim(this.users?.datasociete?.uid, '', '', '', '');
    }
    ...
    showInterim(idsociete: string = '', idinterim: string = '', iduser: string = '', statut: string = '', actifs_only: string = '') {
        this.isloading = true;
        this.dataBenef = [];
        this.filteredData = [];
        this.httService.getData(`${environment.api_url}auth/:saveinterim?idsociete=${idsociete}&idinterim=${idinterim}&iduser=${iduser}&statut=${statut}&actifs_only=${actifs_only}`, false, this.users?.access_token || '')
            .toPromise()
            .then((res: any) => {
                ...
                if (res.body.status) {
                    ...
                    this.dataBenef = res.body.data.map((e: any) => {
                        ...
                        return {
                            ...e,
                            lib_agent_absence: `${e?.data_remplace?.datapersonnel?.nom || ''} ${e?.data_remplace?.datapersonnel?.prenom || ''}`,
                            lib_agent_remplacant: `${e?.data_remplacant?.datapersonnel?.nom  || ''} ${e?.data_remplacant?.datapersonnel?.prenom || ''}`,
