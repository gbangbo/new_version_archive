...
export class CompteModalComponent implements OnChanges {
    ...
    validationForm = new FormGroup({
        idstatutpersonnel: new FormControl('', Validators.required),
    ...
    ngOnChanges(changes: SimpleChanges) {
        if (changes['dataLigne'] && changes['dataLigne']?.currentValue) {
            console.log("maj :::", changes['dataLigne']?.currentValue)
            setTimeout(() => {
                this.validationForm.patchValue({
                    idstatutpersonnel: changes['dataLigne']?.currentValue.datastatutpersonnel?.uid,
                    idroles: changes['dataLigne']?.currentValue?.datarole?.uid,
                });
            }, 1000)
        }
    }
    ...
    showStatutPersonnel(idsociete: string = '', idservice: string = '', idstatutpersonnel: string = '', idpersonnel: string = '', idposte: string = '', idfonction: string = '') {
        ...
        this.httService.getData(`${environment.api_url}auth/:save-statut-personnel?idsociete=${idsociete}&idservice=${idservice}&idstatutpersonnel=${idstatutpersonnel}&idpersonnel=${idpersonnel}&idposte=${idposte}&idfonction=${idfonction}`, false, this.users?.access_token || '')
            ...
            .then((res: any) => {
                ...
                if (res.body.status) {
                    console.log("Statut ===", res.body.data)
                    this.dataUser = res.body.data.map((d: any) => {
                        return {
                            label: `${d.datapersonnel.nom} ${d.datapersonnel.prenom}`,
                            emailAgent: d.datapersonnel.emailAgent,
                            value: d.uid
                        }
                    });
                }
    ...
    submitForm() {
        ...
        let payload = {
            ...
            "email": this.dataUser.find((d: any) => d.value == this.validationForm.value.idstatutpersonnel)?.emailAgent,
        }
        // {
//     "action": 1,
