...
@Component({
    ...
    imports: [
        ...
        NzTreeViewModule, AddModalComponent, NzTreeSelectModule],
    ...
    async deleteFolder(e?: any) {
      ...
      <div style="margin-top: 8px;">
        <p style="font-size: 17px; font-weight: 700; color: #0F172A; margin-bottom: 10px;">
          Êtes-vous sûr de vouloir supprimer niveau ?
        </p>
        <p style="font-size: 13px; color: #64748B; margin: 0;">
          Le niveau(${e.name}) sera supprimé de la liste.
        </p>
      </div>
    `,
            icon: 'question',
        ...
        if (result.isConfirmed) {
            let payload = {
                "action": 3,
                "idorganigramme": e.key,
                "idsociete": e?.datasociete?.uid,
                "sigle": e.sigle,
                "libelle": e.libelle,
                "parent": parent?.key || ''
            }
