...
@Component({
    selector: 'app-cles-api',
    imports: [CommonModule, FormsModule, Select2Module, NzSwitchModule, FeatherIconComponent],
...
export class ClesApiComponent implements OnInit {
    ...
    // Domaines autorisés (select2 multiple)
    ...
    modalError = '';
    form: {
        name: string; header_name: string; key: string; is_active: boolean;
        allowed_domains_uids: string[]; description: string;
    ...
    private mapRow(e: any): CleApiRow {
        ...
        return {
            ...
            revealed: false, revealedKey: '', revealing: false, raw: e,
        };
    }
    ...
    toggleReveal(row: CleApiRow): void {
        if (row.revealed) { row.revealed = false; return; }
        if (row.revealedKey) { row.revealed = true; return; }
        row.revealing = true;
        this.httService.getData(
    ...
    submit(): void {
        this.modalError = '';
        if (!this.form.name.trim()) { this.modalError = 'Veuillez saisir le nom de la clé.'; return; }
        if (!this.isEdit && !this.form.key.trim()) { this.modalError = 'Veuillez saisir ou générer la clé.'; return; }
    ...
    toggleActive(row: CleApiRow): void {
        ...
        const payload = {
            action: 2, uidapikey: row.uid, name: row.name, header_name: row.header_name,
            is_active: !row.is_active, allowed_domains_uids: row.allowed_domains_uids || [], description: row.description,
        };
        this.httService.postData(`${environment.api_url}api/:io-api-keys`, payload, this.users?.access_token || '')
            .toPromise()
            .then((res: any) => {
    ...
    async remove(row: CleApiRow): Promise<void> {
        const result = await Swal.fire({
            html: `<div style="margin-top:8px;"><p style="font-size:17px;font-weight:700;color:#0F172A;margin-bottom:10px;">Supprimer cette clé API ?</p><p style="font-size:13px;color:#64748B;margin:0;">« <strong>${row.name}</strong> » sera définitivement supprimée.</p></div>`,
        ...
        const payload = {
            action: 3, uidapikey: row.uid, name: row.name, header_name: row.header_name,
            is_active: row.is_active, allowed_domains_uids: row.allowed_domains_uids || [], description: row.description,
        };
        this.httService.postData(`${environment.api_url}api/:io-api-keys`, payload, this.users?.access_token || '')
            .toPromise()
