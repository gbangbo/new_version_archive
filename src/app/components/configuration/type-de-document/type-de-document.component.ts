...
@Component({
    ...
    imports: [
        ...
        SvgIconComponent, CreerComponent, ListeComponent],
    providers: [],
...
export class TypeDeDocumentComponent implements OnInit {
    ...
    public settingTabs = [
        {
            id: 1,
            title: 'Creer un type de document',
            value: 'general',
            icon: 'editors',
        },
        {
            id: 2,
            title: 'Voir un type de document',
            value: 'activation',
    ...
    typedocuments(idsociete: string = '', idtypedocuments: string = '') {
        ...
        this.httService.getData(`${environment.api_url}auth/:savetypedocuments?idsociete=${idsociete}&idtypedocuments=${idtypedocuments}`, false, this.users?.access_token || '')
            .toPromise()
            .then((res: any) => {
